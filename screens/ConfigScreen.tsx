...
const ConfigScreen = () => {
  ...
  const handleEquipmentTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const newType = value === 'Autoclave' ? QualificationTypeEnum.AUTOCLAVE : QualificationTypeEnum.CHAMBER;
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 lg:p-8 bg-brand-surface rounded-xl shadow-lg">
        ...
        <div className="space-y-8">
            ...
            <div>
                <h3 className="text-lg font-semibold text-brand-primary mb-4">Detalhes da Qualificação</h3>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <SelectField label="Tipo de Qualificação" name="qualificationPhase" value={state.config.qualificationPhase} onChange={handleChange}>
                        <option value="QI">QI - Qualificação de Instalação</option>
                        <option value="QO">QO - Qualificação de Operação</option>
                        <option value="QP">QP - Qualificação de Performance</option>
                    </SelectField>
                    <SelectField label="Tipo de Cadeia" name="chainType" value={state.config.chainType} onChange={handleChange}>
                        <option value="Fria">Cadeia Fria</option>
                        <option value="Quente">Cadeia Quente</option>
                    </SelectField>
                </div>
            </div>
            ...
            <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <SelectField label="Tipo de Equipamento" name="equipmentType" value={state.config.equipmentType} onChange={handleEquipmentTypeChange}>
                    ...
                    <InputField label={state.qualificationType === QualificationTypeEnum.CHAMBER ? "Temperatura Alvo (°C)" : "Temperatura de Esterilização (°C)"} name="targetTemperature" value={state.config.targetTemperature} onChange={handleChange} />
                </div>
            </div>
